...
def category_servise(request, hierarchy=None):
	...
	parent = None
	category_all = CategoryService.all()

	for slug in category_slug[:-1]:
		parent = category_all.get(parent=parent, slug = slug)
	try:
		instance = CategoryService.get(parent=parent, slug=category_slug[-1])
	except:
		...
		category = CategoryService.get(slug=category_slug[-1])
		services = service.objects.filter(category=category)
		return render(request, 'service/category_service.html', {'instance':instance, 'category':category, 'services': services, 'category_all':category_all})