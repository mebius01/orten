...
# Если ключ есть обновляем существующие данные
    while c < len(raw_product):
        try:
            vendor_code_key = data_list.pop(
                str(raw_product.iloc[c, 0]).split('.')[0])
            vendor_code_key.update(
                {'price': str(round(float(raw_product.iloc[c, 3]), 2))})
...
# 5 Если ключа нет, формируем новый файл для импорта с новыми товарами
            ...
            try:
                ...
                vendor_code = str(vendor_code).split('.')[0]
                slug = slugify(name+'-'+vendor_code)
                price = str(raw_product.iloc[c, 3])
                price = price.replace(",", ".")
                provider = prov
                available, stock = "1", "1"
